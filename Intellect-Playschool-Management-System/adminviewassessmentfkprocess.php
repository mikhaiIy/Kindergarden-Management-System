...
if( mysqli_num_rows($resultVAK)>0){
    header("location:adminviewvakassessmentfk.php?id=".$aid);
}
if(mysqli_num_rows($resultINT)>0) {
    header("location:adminviewintelligentassessmentfk.php?id=".$aid);
}
if(mysqli_num_rows($resultSUB)>0) {
    header("location:adminviewsubjectassessmentfk.php?id=".$aid);
}
